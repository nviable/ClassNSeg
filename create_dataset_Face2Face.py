...
parser.add_argument('--input_real', default ='/home/js8365/data/Sandbox/dataset-deepfakes/FaceForensics/media/original_sequences/c23/videos')
parser.add_argument('--input_fake', default ='/home/js8365/data/Sandbox/dataset-deepfakes/FaceForensics/media/manipulated_sequences/Face2Face/c23/videos')
parser.add_argument('--input_map', default ='/home/js8365/data/Sandbox/dataset-deepfakes/FaceForensics/dataset/splits')
parser.add_argument('--mask', default ='/home/js8365/data/Sandbox/dataset-deepfakes/FaceForensics/media/manipulated_sequences/Face2Face/raw/masks')
parser.add_argument('--output', default ='/home/js8365/data/Sandbox/dataset-deepfakes/FaceForensics/classnseg')
parser.add_argument('--imageSize', type=int, default=256, help='the height / width of the input image to network')
parser.add_argument('--limit', type=int, default=10, help='number of images to extract for each video')
parser.add_argument('--scale', type=float, default =1.3, help='enables resizing')
...
def to_bw(mask, thresh_binary=10, thresh_otsu=255):
    ...
    thresh, im_bw = cv2.threshold(im_gray, thresh_binary, thresh_otsu, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    # print(np.shape(mask), np.shape(im_gray), np.shape(im_bw))
    return im_bw
...
def get_bbox(mask, thresh_binary=127, thresh_otsu=255):
    ...
    # print("Contours: ", np.shape(contours), " locations:", np.shape(locations), " max-idexof: ", np.shape(locations[:,4]))
            ...
            if f.lower().endswith(('mp4')):
                ...
                while (success_real and success_fake):
                    ...
                    if (original_cropped is not None) and (altered_cropped is not None) and (mask_cropped is not None):
                        ...
                        cv2.imwrite(os.path.join(output, output_subfolder, 'original', filename[0:3] + "_%d.jpg" % count), original_cropped)
