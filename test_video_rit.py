...
if __name__ == '__main__':
    ...
    preds = []
    count = 1
    # python test_video_rit.py --outf checkpoints/full --test_video predict2.mp4 --out_video output/predict2-y46.mp4
    while True:
        ret, frame = vc.read()
        if not ret:
            break
        ...
        # not_blob = cv2.resize(frame, (300, 300))
        boxes = face_recognition.face_locations(frame, model="cnn")  # (startY, endX, endY, startX)
        # have to subtract mean  - np.array([103.93, 116.77, 123.68])
        ...
        for i in range(0, len(boxes)):
            ...
            (startY, endX, endY, startX) = boxes[i]
            ...
            # crop face areas
            # crop_img = np.asarray( - np.array([103.93, 116.77, 123.68]), dtype=np.uint8)
        ...
        if len(crop_img_lst) > 0:
            ...
            if one[0] >= zero[0]:
                output_pred[0] = 1.0
                preds.append((one[0].item(),1))
            else:
                output_pred[0] = 0.0
                preds.append((zero[0].item(),0))
